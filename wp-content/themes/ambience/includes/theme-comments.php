...
<?php // if (get_comment_type() == "comment"){ // If you wanted to separate comments from pingbacks ?>
   <li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
      ...
      <?php if(get_comment_type() == "comment"){ ?>
          ...
          <?php } ?>
          <div class="user-meta">
                  <strong class="name"><?php the_commenter_link() ?></strong> <?php if(get_comment_type() == "comment"){ echo get_comment_date("j F Y") ?> <?php _e('at',woothemes); ?> <?php echo get_comment_time(); edit_comment_link(__('Edit',woothemes), ' <span class="edit-link">(', ')</span>'); ?> <a class="comment-permalink" href="<?php echo get_comment_link(); ?>" title="<?php _e('Direct link to this comment',woothemes); ?>">#</a><?php }?>
          </div>
      </div>
      <div class="comment-entry"  id="comment-<?php comment_ID(); ?>">
                                <?php comment_text() ?>
                                <?php if ($comment->comment_approved == '0') echo "<p class='unapproved'>" . _e('Your comment is awaiting moderation.',woothemes) . "</p>\n"; ?>
                                <div class="reply">
