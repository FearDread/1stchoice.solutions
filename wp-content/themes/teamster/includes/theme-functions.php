...
- Woo Get Post Count by Post Type
...
if (!function_exists( 'woo_tabs_popular')) {
	function woo_tabs_popular( $posts = 5, $size = 45 ) {
		global $post;
		$popular = get_posts( 'suppress_filters=0&ignore_sticky_posts=1&orderby=comment_count&showposts='.$posts);
...
<p class="post-meta">
    ...
    <span class="post-author"><?php _e( 'Written by', 'woothemes' ) ?> <?php the_author_posts_link(); ?></span>
...
?>
	...
	<div id="connect">
		<h3 class="title"><?php if ( $title ) echo apply_filters( 'widget_title', $title ); else _e('Subscribe','woothemes'); ?></h3>
...
function woo_custom_profile_fields( $user ) { ?>
	...
	<table class="form-table">
		...
		<tr>
			<th><label for="teamtitle"><?php esc_attr_e( 'Team Title', 'woothemes' ); ?></label></th>

			<td>
				<input type="text" name="teamtitle" id="teamtitle" value="<?php echo esc_attr( get_the_author_meta( 'teamtitle', $user->ID ) ); ?>" class="regular-text" /><br />
				<span class="description"><?php esc_attr_e( 'Please enter your Team Title.', 'woothemes' ); ?></span>
			</td>
		</tr>
		...
		<tr>
			<th><label for="dribbble"><?php esc_attr_e( 'Dribbble', 'woothemes' ); ?></label></th>
			...
			<td>
				...
				<span class="description"><?php esc_attr_e( 'Please enter your Dribbble username.', 'woothemes' ); ?></span>
			</td>
		</tr>
...
function woo_save_custom_profile_fields( $user_id ) {
	...
	update_user_meta( $user_id, 'teamtitle', $_POST['teamtitle'] );
...
function woo_get_users($users_per_page = 10, $paged = 1, $role = '', $orderby = 'login', $order = 'ASC', $usersearch = '' , $meta_query = array() ) {
...
function woo_count_user_posts_by_type($userid, $post_type='post') {
  global $wpdb;
...
/*-----------------------------------------------------------------------------------*/
/* Woo Get Author Posts Url Feed */
/*-----------------------------------------------------------------------------------*/

function woo_get_author_posts_url_feed($author_id, $author_nicename = '') {
	global $wp_rewrite;
    ...
    if ( empty($link) ) {
            $file = home_url( '/' );
            $link = $file . '?author=' . $auth_ID . '&?feed=rss2';
    } else {
            if ( '' == $author_nicename ) {
...
<p class="post-meta">
    <span class="post-date"><?php the_time( get_option( 'date_format' ) ); ?></span>
    <span class="post-author"><?php _e( 'Written by', 'woothemes' ) ?> <?php the_author_posts_link(); ?></span>
    <span class="post-category"><?php the_terms( $post->ID, 'portfolio-gallery', '', ', ', '' ); ?></span>
</p>
<?php
...
function woo_portfolio_image_dimensions ( $layout = 'one-col', $width = '960' ) {
	...
	$dimensions = array( 'width' => 600, 'height' => 0, 'thumb_width' => 179, 'thumb_height' => 150 );
...
/*-----------------------------------------------------------------------------------*/
/* The ajax actions and functions for the ajax author calls. */
/*-----------------------------------------------------------------------------------*/

add_action( 'wp_ajax_author_slider', 'author_slider_ajax_response' );
add_action( 'wp_ajax_nopriv_author_slider', 'author_slider_ajax_response' ); 
// THE FUNCTION
function author_slider_ajax_response(){
	
	$nonce = $_POST['ajax_author_slider_nonce'];
	
	//Add nonce security to the request
	if ( ! wp_verify_nonce( $nonce, 'ajax_author_slider_nonce' ) ){
        die ();
    }    
	//Just get the template part, a check is made there for the author id
	get_template_part( 'includes/author-sorter-slide' ); 
	
	die();  // wordpress may print out a spurious zero without this � can be particularly bad if using json
	
}// End author_slider_ajax_response()
