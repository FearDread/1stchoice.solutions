...
/*-----------------------------------------------------------------------------------*/
	...
	if ( ! function_exists( 'woo_comment_form_fields' ) ) {
		function woo_comment_form_fields ( $fields ) {
			...
			$fields =  array(
				'author' => '<div class="fl"><p class="comment-form-author">' .	
							'<input id="author" class="txt" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="45"' . $aria_req . ' />' .
							'</p>',
				'email'  => '<p class="comment-form-email">' .		
				            '<input id="email" class="txt" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="45"' . $aria_req . ' />' .
				            '</p>',
				'url'    => '<p class="comment-form-url">' .		
				            '<input id="url" class="txt" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="45" />' .
				            '</p></div>',
			);
...
function woo_featured_slider_get_slides ( $args ) {
	...
	if ( 0 < intval( $args['term'] ) ) {
		$query_args['tax_query'] = array(
										array( 'taxonomy' => 'post_tag', 'field' => 'id', 'terms' => intval( $args['term']) )
									);
	}
...
<aside class="portfolio-meta">
	<ul>
		...
		<li class="portfolio-comments">
			<?php comments_popup_link( __( 'Leave a comment', 'woothemes' ), __( '1 Comment', 'woothemes' ), __( '% Comments', 'woothemes' ), 'button' ); ?>
		</li>
...
add_action('comment_form_before', 'my_container_c');
function my_container_c() {
	echo '<div class="inner">';
}

add_action('comment_form_after', 'my_container_d');
function my_container_d() {
	echo '</div>';
}
