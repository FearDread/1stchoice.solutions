...
if ( jQuery( '.layout-group .section' ).length ) {
	...
	jQuery( '.layout-group .section' ).each( function ( index, element ) {
		...
		jQuery( this ).find( 'input.woo-input' ).each( function ( index, element ) {
			...
			divHtml += '<div id="' + divId + '" class="' + divClass + '"><span class="content">' + columnName + '<small>(approx. <span class="pixel-width">' + '' + '</span>px)</small></span></div>';
		...
		if ( jQuery( layoutDiv ).find( 'div' ).length >= 1 ) {
			...
			var westWidth = jQuery( this ).find( '.controls input.woo-input:eq(0)' ).val();
			var eastWidth = jQuery( this ).find( '.controls input.woo-input:eq(2)' ).val();
			
			// Work out the pixel widths for the various columns.
			var onePercent = parseInt( layoutDiv.width() ) / 100;
			
			var westWidth = Math.ceil( onePercent * westWidth );
			var eastWidth = Math.ceil( onePercent * eastWidth );
			var centerWidth = parseInt( currentLayoutWidth ) - westWidth - eastWidth;
		...
		jQuery( this ).find( 'div:eq(0) .pixel-width' ).text( westWidth );
		jQuery( this ).find( 'div:eq(1) .pixel-width' ).text( centerWidth );
		jQuery( this ).find( 'div:eq(2) .pixel-width' ).text( eastWidth );
...
function update_dimensions ( element ) {
	...
	layoutParent.children( 'div' ).each( function () {
		...
		jQuery( this ).find( '.pixel-width' ).text( newPixelWidth );
